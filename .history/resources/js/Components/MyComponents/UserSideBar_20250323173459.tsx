...
const UserSideBar = ({ pageTitle }:any) => {
  ...
  return (
    <>
      <div
        className={`2xl:p-8 fixed inset-y-0 left-0 flex-wrap items-center justify-between overflow-y-auto transition-all duration-200 bg-white z-40 border-0 shadow-xl xl:p-6 max-xl:px-6 max-xl:w-72 max-xl:shadow-2xl max-xl:min-h-full ease-soft-in-out w-1/6 xl:translate-x-0  ${
          isMenuOpening ? `` : `-translate-x-full`
        }`}
      >
        <div
          className="top-2 -right-1 z-50 bg-green-500 text-grE w-10 h-10 hidden max-xl:flex rounded-s-full flex absolute"
